...
سینمچ از فیلم‌ها و امتیازهای واقعی MovieLens و یک کاتالوگ مستند سینمای ایران، پیشنهاد توضیح‌پذیر می‌سازد. خروجی از طریق HTTP در اختیار رابط فارسی React + Vite قرار می‌گیرد. پروژه برای اجرای محلی و دفاع دانشگاهی طراحی شده است؛ استقرار توزیع‌شده، صف پیام و سامانهٔ هویت بیرونی عمداً خارج از محدوده‌اند.

## اجزا

```text
مرورگر
  └─ React + Vite: رابط RTL، فرم‌ها و وضعیت نشست محلی
       └─ HTTP/JSON
...
۴. `expand_movie_catalog.py` در صورت درخواست، کاتالوگ MovieLens 32M را پس از بررسی checksum وارد می‌کند و تعداد فیلم‌ها را به ۸۷٬۹۰۳ می‌رساند؛ امتیازهای ۳۲ میلیونی عمداً وارد نمی‌شوند.
۵. ثبت‌نام یک هش scrypt نمک‌دار می‌سازد و ورود، توکن امضاشده و زمان‌دار برمی‌گرداند.
۶. ثبت امتیاز، امضای cache را تغییر می‌دهد تا ساختار مدل در درخواست بعدی تازه شود.
۷. فرم هوشمند، حال‌وهوا و ژانر را به بردار TF–IDF تبدیل می‌کند؛ شباهت کسینوسی، دوره و کشور، نامزدها را رتبه‌بندی می‌کنند.
۸. مدل دارای سابقه، امتیاز مشارکتی و محتوایی را نرمال و ترکیب می‌کند و فیلم‌های دیده‌شده را کنار می‌گذارد.

...
export default function Tracker({ user, navigate, onDetails, showError }) {
  ...
  const updateEntry = async (entry, payload) => {
    try {
      ...
      setEditing(null)
      setNotice('پیشرفت تماشا ذخیره شد.')
      ...
      <div className="heatmap-scroll" tabIndex="0">
        <div className="heatmap-chart" role="img" aria-label={`${COPY.tracker.activityTitle}؛ ${faNumber(activity.total_units)} تماشا در ${faNumber(activity.active_days)} روز`}>
          ...
          <div className="heatmap" aria-hidden="true">
            {activity.days.map((day) => <span key={day.date} className={`activity-cell level-${day.level}`} title={COPY.tracker.activityLabel(day.date, faNumber(day.count))} />)}
          </div>
        </div>
      </div>
...
function LibrarySection({ status, title, entries, empty, navigate, onDetails, onEdit, onRemove, onUpdate }) {
  ...
  return (
    <section className="library-section">
      <SectionTitle eyebrow={<><Icon aria-hidden="true" />{faNumber(entries.length)} عنوان</>} title={title} />
      {entries.length ? <div className="library-list">{entries.map((entry) => <LibraryRow key={entry.entry_id} entry={entry} onDetails={onDetails} onEdit={onEdit} onRemove={onRemove} onUpdate={onUpdate} />)}</div> : <Empty title={empty} text="از آرشیو، عنوان دلخواهت را به این بخش اضافه کن." />}
...
function LibraryRow({ entry, onDetails, onEdit, onRemove, onUpdate }) {
  const serial = entry.media_type === 'serial'
  return (
    <article className="library-row">
      <button type="button" className="library-artwork" onClick={(event) => onDetails(entry, event)} aria-label={`جزئیات ${entry.display_title}`}><CardArtwork movie={entry} index={0} /></button>

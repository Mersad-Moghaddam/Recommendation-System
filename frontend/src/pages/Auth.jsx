...
export default function Auth({ onAuthenticated }) {
  ...
  const validateField = (name, value) => {
    ...
    if (!trimmedValue) return name === 'username' ? COPY.auth.usernameRequired : COPY.auth.passwordRequired
    if (name === 'username' && trimmedValue.length < 3) return COPY.auth.usernameShort
    if (name === 'password' && value.length < 6) return COPY.auth.passwordShort
    return ''
  }

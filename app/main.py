...
def movies(q: str = "", skip: int = 0, limit: int = Query(24, ge=1, le=100), persian_only: bool = False, db: Session = Depends(get_db)):
    return [movie_out(m) for m in MovieService.list(db, q, skip, limit, persian_only)]
...
@app.get("/users/me/ratings", response_model=list[RatingOut], tags=["امتیازها"])
